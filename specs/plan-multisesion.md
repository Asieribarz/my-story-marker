# plan-multisesion.md — Lo que cada sesión tiene que arreglar
...
- [ ] **media** · `.claude/harness/politica.py:133` — La policy solo mira el nombre del `path` de Read y Grep: un Grep sobre un directorio, o con `glob`, lee el texto adversarial de E2. El brief E2 y plan-agentes §5.3 afirman que no se puede leer.
  - Arreglo: Denegar Grep cuando `glob` o `pattern` apunten a `texto_libre`, o cuando `path` sea un directorio que contenga esos ficheros; o sacarlos del alcance de ripgrep con un `.ignore` para `evals/*/input/texto_libre*.txt`. Ajustar plan-agentes §8 y, en la sesión de briefs, la redacción de e2 L10 y L119.
...
Avisos del backend al cerrar los bloques 2 y 3 (2026-09-24, spec-backend-2 §4.5):
...
- [x] **aviso** · `bibliotecario.md` pide registrar relaciones y no existe esa escritura: quitarlo del prompt (herramientas en spec-backend-1 §5.2).
...
Salido de la ejecución de E5 (2026-09-24, [iteraciones.md](../docs/iteraciones.md) I-08, [evals.md](../docs/evals.md)):

- [ ] **alta** · `.claude/agents/agente-contexto.md:33` — El Trabajo 2 resuelve por su cuenta las contradicciones del comprador (edad frente a fecha de nacimiento, tono frente a edad): «ninguna regla de coherencia falla» pesa más que «lo que el comprador fijó, se respeta tal cual». E5 falla por esto.
  - Arreglo: Cuando un valor fijado por el comprador incumple una regla, dejarlo tal cual y nombrarlo en una nota, para que lo marque el validador. Las reglas de coherencia solo gobiernan lo que se deriva. Es el ajuste v1 → v2 de E5; después se repite E5 con un proyecto nuevo.
- [ ] **media** · `.claude/skills/entrevista/SKILL.md:33` — `/entrevista` solo vuelve a preguntar al comprador si el proyecto sigue en `intake`. Con los hallazgos de RF-24 sobre valores del comprador, el proyecto está en `contexto` y no hay pregunta.
  - Arreglo: Cuando el backend pare en `contexto` con hallazgos sobre valores del comprador (ver la fila de backend de I-08), enseñárselos, preguntar y reenviar la corrección. Depende del arreglo de backend.

## Sesión de briefs (evals/)

- [ ] **media** · `evals/e2-injection/input/brief.json:96` — La señal del hook de policy en E2 espera que se deniegue leer proyecto.sqlite, pero ni la policy ni su especificación lo deniegan con Read o Grep, y la decisión no llega a la tabla auditoria.
...
- [ ] **media** · `evals/e2-injection/input/brief.json:90` — La señal del identificador de un solo uso exige que cada intento quede en llamada_mcp. Hoy /mcp/entrada no registra nada; para el identificador inyectado no hay base donde escribirlo; y registrarlo como pide RF-105 contradiría la señal de la L102.
...
- [ ] **baja** · `evals/e3-temporal-inconsistency/input/brief.json:87` — La descripción de E3 dice que lo que pase de la ficha al texto solo lo ve Lean, en contra de R-2 y del propio puede_saltar del brief.
...
- [ ] **baja** · `evals/e3-temporal-inconsistency/input/brief.json:119` — El caso de control «Lean sobre h2» no controla nada, porque nadie registra al ser querido de h1 como presente en el evento h2.
...
- [ ] **alta** · `backend/contexto/validacion.py` — (E5, 2026-09-24, I-08) Nada comprueba que el contexto conserve lo que dio el comprador, y en `contexto` no hay parada humana: un hallazgo de RF-24 sobre un valor del comprador vuelve al Agente de Contexto, que puede «corregirlo» sin preguntar.
  - Arreglo, a decidir por la persona: (1) una regla del validador que compare `personalizacion` del contexto con el brief normalizado y marque cualquier cambio en el destinatario, `edad_lector` o los hechos, salvo el `lugar` de los eventos, que es lo que `evals/comprobar.py` exige a los contextos de referencia; (2) que los hallazgos sobre valores del comprador lleven a `esperar_humano`, no a un reintento del agente. (2) toca el grafo de estados: la sesión formal lo refleja en la spec TLA+.

- [ ] **alta** · `specs/spec-backend-2.md:111` — El cuerpo de /resultado se contradice dentro de §4.1: el punto 1 dice que el campo es `sello` y el punto 7, que es `orden`.
...
- [ ] **alta** · `specs/spec-backend-2.md:152` — La lista de §4.4 deja fuera los recortes R-1, R-2 y R-3, que según §0 mandan sobre todo lo demás, aunque dejan desfasados architecture, spec-backend-1, validators y plan-entrega.
  - Arreglo: Añadir a §4.4 una entrada por recorte, con estos destinos: R-1 → architecture §3, §3.1 y §11, spec-backend-1 §1.2 y plan-entrega §4 y H2; R-2 → architecture §4 y §4.2, spec-backend-1 RF-74, RF-75, V-12 y V-17, y validators V-12; R-3 → plan-entrega E-5, H3 y H9.
...
- [ ] **media** · `specs/spec-backend-2.md:43` — B-14 no define qué es un párrafo, así que p<n> del backend, data-p del frontend y 'parrafo' del juez pueden desalinearse
...
- [ ] **media** · `specs/spec-backend-2.md:18` — R-4 mueve el proyecto a publicada con el cambio fallido cuando falla el worker, pero esa transición no está ni en TRANSICIONES ni en Aristas, y la spec deja el worker fuera del modelo.
...
- [ ] **media** · `specs/spec-backend-2.md:112` — El sello de §4.1.2 y §4.1.3 es el antiguo: dos segmentos y repetición obligatoria. Además, ningún punto de decisiones fija dónde viaja el sello en /siguiente, y el harness lo resuelve con un fallback silencioso.
...
- [ ] **media** · `specs/spec-backend-2.md:152` — §4.4 también deja fuera otros cambios de docs/ que decisiones exige explícitamente.
  - Arreglo: Añadir a §4.4: definitions §3 y spec-backend-1 RF-90 (§3.8); architecture §7, AGENTS.md y spec-backend-1 D-8 (TC-6); architecture §2, §4 y §6.2 (§3.13); architecture §8 hooks y skill (§4.1.5); architecture §6 (B-2); architecture §3.1 y spec-backend-1 RF-08 (TC-3/AJ-5); y AGENTS.md «Comprobaciones», con las pruebas de .claude/tests (plan-agentes §9).
- [ ] **media** · `specs/spec-backend-2.md:30` — B-18 no cuadra con B-1: la materialización al salir de `contexto` escribe `evento.lugar` y `evento.excluye`, que son claves ajenas a `localizacion` y `personaje`, pero B-1 no materializa ni personajes ni localizaciones. Tampoco dice cómo se traduce `fuente` (un id de hecho) a un id de personaje.
...
- [ ] **media** · `specs/spec-backend-2.md:96` — El fichero Lean se guarda por ciclo (`verificacion/ciclo-k/`), pero AJ-3 decide que los gates se guardan por pasada precisamente porque el ciclo se reinicia y se reutilizaría el Lean de una versión anterior.
...
- [ ] **baja** · `specs/spec-backend-2.md:154` — La lista §4.4 de cambios de documentos pendientes no recoge R-1 ni el registro de todas las salidas por el hook (P-2), así que architecture y spec-backend-1 seguirán describiendo 13 agentes y dos vías de registro
  - Arreglo: Añadir a §4.4: architecture §3 (12 agentes, planificador), §3.1 (una sola orden en planificacion), §8 (SubagentStop registra todas las salidas; la skill solo lee el acuse) y spec-backend-1 l.56.
...
- [ ] **baja** · `specs/spec-backend-2.md:89` — decisiones §3 punto 2 y el código y la spec no dicen lo mismo sobre en qué estado actúa el Bibliotecario.
...
- [ ] **baja** · `specs/spec-backend-2.md:96` — El fichero Lean se guarda por ciclo (verificacion/ciclo-k/), mientras que AJ-3 decide que los gates se guarden por pasada porque los ciclos se repiten.
...
- [ ] **baja** · `specs/spec-backend-2.md:128` — El contrato Lean exige una franja (`manana`, `tarde`, `noche`) en cada evento de la historia, pero ninguna decisión dice de dónde sale: la ficha y el Bibliotecario solo manejan días.
...
- [x] **baja** · `docs/red-team.md:11` — red-team.md dice que E2 son dos ataques (dos entradas), y el brief E2 define cuatro vectores y pide una entrada por vector.
...
Salido de las novelas de E3 y E4 (2026-09-24, [iteraciones.md](../docs/iteraciones.md) I-11):

- [ ] **alta** · `formal/lean/Cronologia` y la cronología del Bibliotecario — «lugar único» da falsos positivos por desplazamientos dentro de una misma franja: el momento no tiene orden dentro de la franja. Bloquea publicar E3 y E4.
  - Arreglo (a decidir): un orden dentro de la franja (secuencia del evento en su día) o tratar como compatibles los eventos de la misma franja en lugares distintos cuando hay desplazamiento; o que el Bibliotecario registre la franja siguiente al moverse.
- [ ] **alta** · `backend/mcp/lectura.py:289` y `.claude/agents/juez-capitulo.md` — `leer_capitulo` exige versión e intento, y el juez lee un capítulo anterior con el intento de su propio capítulo: puede citar una versión rechazada.
  - Arreglo (a decidir): que `leer_capitulo` sin `version`/`intento` devuelva el texto vigente, y que el prompt del juez lo use para capítulos anteriores.
- [ ] **media** · `.claude/agents/bibliotecario.md` — al reescribir un capítulo de un acto con resumen, olvida `escribir_resumen_acto` (B-17) y gasta un intento.
- [ ] **media** · `.claude/skills/orquestar-novela/SKILL.md` y `backend/cambio/worker.py` — en `claude -p` sin preguntas, la expansión `${CLAUDE_PROJECT_DIR:-.}` se deniega y en Windows el orquestador elige PowerShell. El worker de `/regenerar` fallaría igual.

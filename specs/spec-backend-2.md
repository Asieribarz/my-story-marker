# spec-backend-2.md — Decisiones de los tramos B y C del backend
...
| **B-13** | **E-5 aceptada:** verificador de Repeticiones en severidad **baja** (la misma palabra de contenido de 4+ letras 3 veces en un párrafo o en 50 palabras; tres frases seguidas con la misma primera palabra). Umbrales como constantes. RF y V nuevos en spec-backend-1. | 7 |
...
| **B-20** | **Conservación del brief** (2026-09-24, tras E5 y E4): al registrar la normalización y el contexto, el backend compara la salida del Agente de Contexto con las respuestas guardadas y los hechos confirmados, y la rechaza como fallo de contenido si cambia lo que fijó el comprador o trae un marcador de anonimización (`[NOMBRE_ANONIMIZADO]`…) que no venía del brief. `ocasion`, `relacion` y `papel` solo se exigen iguales si ya eran un valor permitido, y el `lugar` de un hecho puede pasar a id. La evidencia nombra la clave, no el valor (RF-13). Código: `contexto/conservacion.py`, que también da el patrón de marcador al verificador de prosa. | contexto |
...
| **TC-2** | Igual que B-6. La invariante 3 queda como «nadie aparece en un evento fechado antes de nacer»; la v1 no compara edades explícitas, y se dice en spec-backend-1. | 8, 8a |
...
8. **Macroestructura.** En la v1, solo la titulación con valores cerrados (`numerado`, `titulado`, `numerado_y_titulado`); prólogo, epílogo e interludios quedan fuera del alcance, y se dice en definitions y spec-backend-1.
...
- **spec-backend-1**: V-15 (R-5), D-7 sin cosmic-ray, §1.2 (criterio `contenido` del juez, B-15), la invariante 3 de Lean (TC-2), V-32 con suma ≥ 18 (TC-5) y §5.1 (TC-12, más las rutas de auditoría y de cambios).
- **plan-entrega H0**: los briefs son JSON.
- **spec-backend-1 RF-05a**: `cambio_solicitado` solo sale por acción humana, **salvo** cuando el Intérprete agota su tope: entonces vuelve a `publicada` con el cambio fallido (Q6). Lo detectó la revisión del bloque 1.
- **spec-backend-1 RF-14**: el identificador es `<proyecto>.<secreto>`. Lo que «no se deriva del proyecto» es el **secreto**; el prefijo solo sirve para que la petición identifique su proyecto sin recorrer los demás (V-24). Lo detectó la revisión del bloque 1.
- **architecture §3.1**: las aristas de Q6 (13 pares) entran en el diagrama y en la tabla.
- **R-1** (12 agentes, una sola orden en `planificacion`): architecture §2 y §3 (tabla y diagrama de agentes, y la fila de `planificacion` de §3.1), spec-backend-1 §1.2 y §4.4, y plan-entrega §4.
- **R-2** (sin heurísticas sobre la prosa): spec-backend-1 RF-74 y RF-75 (solo antes de escribir), V-12 y V-17 en spec-backend-1 §9 y validators.md.
...
- **B-8** (bloque «Informe del intento anterior» y vacíos opcionales): architecture §6.3, en la tabla de bloques, y spec-backend-1 RF-50c.
- **§4.1** (cuerpo de `/resultado`, sello y `/auditoria`): spec-backend-1 §5.1.

...
> - [specs/spec-backend-1.md](../specs/spec-backend-1.md) §9 es el **criterio de aceptación por requisito** del backend v1: `V-1` a `V-35`. Cuando una fila de aquí tiene un `V-n`, ese es su enunciado exacto y este documento no lo reescribe.
>
> La dirección importa: si las dos vistas discrepan, manda `spec-backend-1.md`, porque un criterio de aceptación es un compromiso y esto es su mapa.
...
| La salida del Agente de Contexto conserva lo que fijó el comprador (B-20): un valor cambiado o tapado con un marcador de anonimización se rechaza en la normalización y en el contexto, y lo que se deriva o se traduce no | Una prueba por regla que la dispara y otra de control (`contexto/tests/test_conservacion.py`), y dos de integración sobre el grafo, una por paso (`proyecto/tests/test_contexto_en_grafo.py`) | T | V-15 |
...
La forma de la batería la fija **D-4**, ya resuelta ([spec-backend-1.md](../specs/spec-backend-1.md) §10): Pydantic v2 como fuente única, así que cada regla es un validador de modelo y cada caso inválido, un `ValidationError` con su ruta.
...
| El panel (`GET /proyectos`, `GET /proyectos/{id}/metricas`) solo lista proyectos con base, ordena del más reciente al más antiguo, cuenta un reintento por cada intento > 1, no suma consumo de los intentos sin metadatos, reparte el tiempo por fase según `transicion` y toma la última evaluación del juez | Prueba de integración por la API, una que dispara cada regla y su control (`proyecto/tests/test_panel.py`) | T | — |
...
La legibilidad es el índice de Szigriszt-Pazos con la escala INFLESZ (**D-6**, resuelta en [spec-backend-1.md](../specs/spec-backend-1.md) §10). Como se calcula en código propio, el conteo de sílabas es código nuestro y necesita sus propias pruebas con palabras de silabeo conocido.
...
| La generación desde la web solo se encola fuera de las paradas humanas, no se duplica, y si falla deja el proyecto donde estaba con el bloqueo suelto | Prueba de la ruta y del worker con el `claude` falso (`cambio/tests/test_generacion.py`) | T | TC-8, R-4 |
...
| Hacia Langfuse no sale texto del brief, de prompts, capítulos, hallazgos, argumentos de herramienta, términos vetados ni justificaciones | Prueba con un centinela escrito en todos esos campos de una base de ensayo: la exportación entera no lo contiene (`backend/observabilidad/tests`) | T | E-1 |

Las tres primeras son `A` y no `T` a propósito: son afirmaciones sobre el **texto del repositorio**, no sobre su comportamiento en ejecución. Ejecutarlo para comprobarlas sería más caro y menos concluyente —una importación que nunca se ejecuta en la ruta probada seguiría ahí—. La cuarta sí es `T`: lo que sale hacia Langfuse depende de los datos de cada base, y solo se ve ejecutando la exportación.

**La tercera fila es nueva y venía de un agujero de este documento.** El aislamiento por proyecto era el único requisito no funcional cuyo método en `spec-backend-1.md` §7 era «por construcción», o sea ninguno, y aquí no tenía fila — mientras §6 lo usaba como argumento para descartar la ejecución en sandbox. Un método descartado apoyándose en una afirmación que nadie sostiene es exactamente el supuesto silencioso que §1 prohíbe. «Por construcción» sí tiene traducción al marco: es `A`, se lee en el repositorio y no hace falta ejecutar nada.
...
| Afirmación | Método | Tipo | Criterio |
|---|---|---|---|
| El contrato de `lectura.json`, las rutas, el filtro HTML, las direcciones de las tres ventanas y el paso del formulario al brief se cumplen | `node:test` | T | — |
| El frontend compila | `vite build` | A | — |
| Las tres ventanas se ven bien (portada, capítulo, métricas, formulario y seguimiento; escritorio y 390 px, claro y oscuro) | Capturas del navegador con Playwright: la MCP, o `playwright` para Python con Edge (`channel="msedge"`) donde no hay Chrome | I | — |
| Crear una novela desde el formulario, seguirla y pedir, ver y confirmar un cambio funcionan contra el backend real | Recorrido con Playwright contra un backend sobre una copia de un proyecto publicado, con el worker apagado (`MSM_WORKER=0`) y el Intérprete simulado por la API. El paso a `publicado` necesita una regeneración real y no entra | D | — |
...
**Esa fila ya tiene vehículo: las trazas de Langfuse** (architecture.md §7, `backend/observabilidad/`), con tokens y latencia por orden, por capítulo y por novela. El coste lo calcula Langfuse con sus precios por modelo; con suscripción es una **estimación**, no un cargo. **Antes de tenerlo:** [architecture.md](architecture.md) §11 pone la observabilidad y las evals en la Fase 1. Conviene distinguir las dos cosas que se confunden aquí: **medir** coste y latencia es esta fila, un `D` sobre trazas; **controlarlos** es elegir el modelo de cada subagente por adelantado, que ya está decidido en `architecture.md` §3 y §10. Lo primero comprueba que lo segundo funciona.

**Ese `D` depende además de D-3.** La observabilidad está sin decidir en [spec-backend-1.md](../specs/spec-backend-1.md) §10. Hasta que se cierre, la fila existe como método elegido pero sin vehículo, y no se introduce nada por iniciativa propia.
...
Un aviso sobre esa lista: **Contenido y líneas rojas no se construye en la v1**, aunque `architecture.md` §4 lo marque bloqueante. La exclusión y su motivo están ahora escritos en [spec-backend-1.md](../specs/spec-backend-1.md) §1.2. Hasta la fase 2, nada comprueba el texto de un capítulo contra los límites de contenido del público objetivo — RF-25 los valida en el objeto de contexto, que es otra cosa.
...
2. Si la afirmación es un requisito del backend v1, su criterio se escribe en [spec-backend-1.md](../specs/spec-backend-1.md) §9 como un `V-n` nuevo, y **aquí** se añade la fila que lo mapea a su pieza.

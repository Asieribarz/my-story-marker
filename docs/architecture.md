...
flowchart LR
  ...
  GEN --> FA{"¿Fin de acto?"}
  FA -->|"sí"| VA["Resumen de acto · Bibliotecario"] --> GEN
  FA -->|"no"| GEN
...
| Bucle por capítulo | Ficha + biblia | Capítulo aprobado + biblia actualizada | Escritor, Editor, verificadores, juez de capítulo, Bibliotecario |
...
| Revisión dirigida | Informe de los gates | Capítulos corregidos, con la biblia al día | Revisor, deterministas y Bibliotecario, por capítulo |
...
La verificación de acto queda fuera de la v1: al cerrar un acto, el Bibliotecario de su último capítulo escribe el resumen del acto (B-17, §6.2).

---

## 3. Agentes y orquestación

Cada agente de esta tabla es un **subagente de Claude Code**, con su propio contexto y el modelo de la columna *Modelo*, que es el valor del campo `model` de su definición (§8). El reparto sigue lo que produce cada agente: `opus` para la prosa, que es el producto; `sonnet` para planificar, extraer los hechos de cada capítulo (Bibliotecario), juzgar y corregir estilo; `haiku` solo donde la salida es corta y la valida un esquema cerrado. Los jueces de §4 son también subagentes (`juez-capitulo` y `juez-manuscrito`, en `sonnet`): un juez más débil que el escritor no detecta lo que tiene que detectar. Con ellos, la novela usa **12 agentes**: la planificación es un solo `planificador` más el `escaletista` (R-1).
...
flowchart LR
  ...
  ORQ --> JC["Juez de capítulo"]
  ORQ --> JM["Juez de manuscrito"]
  EXH -.-> CTX
  CTX -.-> PLA
  PLA -.-> ESC
  ESC -.-> WRI
...
| **Planificador** | En una sola salida JSON (R-1): plan estructural (modelo, hitos por capítulo, curva de tensión, pregunta dramática, final), personajes con arco, relaciones y estado inicial, mundo (localizaciones, ruta, reglas, objetos con su poseedor inicial) y guía de estilo (narrador, métricas, léxico, lista negra, onomástica). Añade al contexto, no lo cambia (B-1). | Contexto + `notas_plan` si el comprador pidió cambios | Plan, personajes, mundo y guía de estilo | `sonnet` |
...
| **Juez de capítulo** | Juzga el capítulo editado por criterio, con la biblia como evidencia; cada hallazgo cita el párrafo (B-15). | Capítulo editado | Informe por criterio | `sonnet` |
| **Bibliotecario** | Extrae del capítulo verificado los hechos nuevos (estado de personajes, objetos, días, presagios) y actualiza la biblia y el resumen, etiquetado con el capítulo de la orden vigente (B-16). | Capítulo verificado | Biblia actualizada | `sonnet` |
| **Juez de manuscrito** | Puntúa la novela entera con la rúbrica de §4.1. | Manuscrito vigente | Cinco puntuaciones con justificación | `sonnet` |
| **Revisor dirigido** | Corrige un capítulo concreto a partir del informe de los gates, con cambios mínimos. | Capítulo + manuscrito vigente + informe de los gates + notas | Capítulo corregido | `opus` |
...
**Quién decide el siguiente paso.** El backend expone la **siguiente orden** de un proyecto. Su elección es una función determinista del estado persistido, y la orden **se persiste al emitirse** —con los identificadores de un solo uso y el fichero de prompt que necesite—, así que pedirla otra vez mientras sigue vigente devuelve la misma. Indica qué subagente lanzar, con qué entrada (el prompt ensamblado o los identificadores que necesita) y dónde registrar el resultado; o bien `esperar_humano`, que el proyecto está en `publicada` o `detenida`, o `error` con una causa cuando la orden no se puede emitir: `no_cabe` (el prompt no cabe ni con la ficha sola, D-9), `inconsistencia` (la continuidad falla antes de escribir, B-9), `lean_no_disponible` o `pdf_no_disponible` (falta la herramienta, TC-3), `cronologia_invalida` (la biblia no se puede convertir en el fichero Lean) o `contexto_incoherente` (Lean falla solo con eventos del contexto, sin ningún capítulo al que mandar el Revisor: nunca se lanza un Revisor sin capítulo) o `revision_sin_capitulos`. Cobertura y Lean corren en una fase previa, fuera de la transacción de escritura, para que Lean no bloquee la base (M-27). Con `error` el estado no cambia y no se gasta intento ni ciclo de revisión (AJ-5). Cada orden lleva su número de intento, con tope 3 para cualquier agente, y un **sello** `<proyecto>:<orden>:<generación>` que la sesión devuelve con el resultado (AJ-4, §3.2). El resultado se registra contra la orden vigente: el backend valida su esquema, lo persiste y **escribe él la transición**; la sesión no escribe transiciones. Reintentos, topes, paradas y `detenida` se deciden ahí, con los contadores que ya viven en la base. La sesión no interpreta el grafo: pide, ejecuta, registra y vuelve a pedir. Así lo más frágil del sistema —un modelo aplicando reglas— pasa a ser código con pruebas, y la especificación TLA+ de §3.4 modela esa función.
...
flowchart LR
  ...
  S1 & S2 & S3 & S4 & S7 & S8 -->|"tope agotado"| D1
  S6 -->|"tope agotado"| D1
  S6 & S7 & S8 & S10 -->|"tope agotado · regeneración"| S9
  S10 & S11 & S6 & S7 & S8 -->|"worker_fallido"| S9
  D1 -->|"reintentar · fase de origen"| S1 & S2 & S3 & S4 & S7 & S8
```
...
| `planificacion` | Planificador: una sola orden (R-1, AJ-1) | Existe plan estructural, fichas, biblia de mundo y guía de estilo | El agente |
...
| `revision` | Por cada capítulo señalado en los gates de la pasada: Revisor → deterministas → Bibliotecario (AJ-2). Nunca se lanza un Revisor sin capítulo | Los capítulos del informe están corregidos y con la biblia al día | El agente; vuelve siempre a verificar |
...
| `cambio_solicitado` | Intérprete de cambios | El comprador confirma o rechaza el cambio propuesto; si el Intérprete agota su tope, vuelve a `publicada` con el cambio fallido | **Humano**, salvo el tope |
...
| `detenida` | Ninguno | El comprador pide reintentar: se reabren los capítulos en `revision_humana` con su contador a cero, se pone a cero el contador de ciclos de revisión y se vuelve a la fase de la que vino (`proyecto.detenida_desde`); desde `verificacion_manuscrito`, a `capitulos` (Q6) | **Humano** |

Además de las aristas de la tabla, la de transiciones (`backend/proyecto/transiciones.py`) tiene las del tope de intentos de una orden que no es de capítulo (Q6): `tope_agotado` lleva a `detenida` desde `intake`, `contexto`, `planificacion`, `escaleta`, `verificacion_manuscrito`, `revision` y `publicacion`, y a `publicada` con el cambio fallido desde `verificacion_manuscrito`, `revision`, `publicacion` y `cambio_solicitado`. Y las de `worker_fallido` (R-4): si el `claude -p` del worker falla durante una regeneración, el proyecto vuelve a `publicada` con el cambio fallido desde cualquiera de los cinco estados que recorre (`cambio_solicitado`, `regeneracion`, `verificacion_manuscrito`, `revision`, `publicacion`).
...
**Estados del capítulo y qué gasta un intento.** El Escritor deja el capítulo en `borrador`; el Editor con los deterministas en verde lo pasa a `editado`; el juez de capítulo en verde, a `verificado`; y el resultado aceptado del Bibliotecario, a `aprobado`, que actualiza `capitulo.version_vigente` (M-3). Hay dos clases de fallo (Q4, Q5): **de contenido** —deterministas altos o bloqueantes, guardarraíl, juez que rechaza o salida inválida del Escritor—, que gasta `capitulo.intentos` y vuelve al Escritor; y **de formato** —salida inválida de un agente posterior al Escritor—, que repite solo ese agente sobre el mismo texto y gasta `proyecto.intentos_paso`.

**Pasadas de verificación (AJ-3).** `proyecto.pasadas` suma 1 cada vez que se entra en `verificacion_manuscrito` y nunca vuelve a cero. Los gates se guardan por pasada (`gate_resultado`), no por ciclo, para que las notas de `aprobacion_final` o un «reintentar» no reutilicen la cobertura y el Lean de una versión anterior. Los capítulos que van a `revision` son los que los gates de la pasada en curso señalan en `detalle.capitulos`.

**El sello lleva la generación del bloqueo (AJ-4).** `proyecto.generacion_bloqueo` sube cada vez que toma el bloqueo un titular distinto o uno caducado, y la orden vigente se vuelve a sellar: si es del Bibliotecario, se borra lo escrito para su capítulo (B-16); si es del Extractor o del Intérprete, se emite un identificador nuevo. `/resultado` y las herramientas de `/mcp/escritura` rechazan un sello viejo, así que un ejecutor caído no duplica la biblia ni gasta un intento. El sello sale en `/siguiente`, no en `GET /estado`.
...
- **No se salta una parada humana activa.** Las paradas `aprobacion_plan` y `aprobacion_final` son configurables por proyecto y están desactivadas por defecto: para un regalo, el punto de control natural del comprador es leer y pedir cambios. Cuando están activas, no tienen transición automática de salida, ni siquiera con todos los verificadores en verde. `publicada`, `cambio_solicitado` y `detenida` tampoco salen sin acción humana, con dos excepciones escritas en la tabla: en `cambio_solicitado`, la parada es la confirmación del cambio ya propuesto, y el Intérprete que agota su tope devuelve el proyecto a `publicada` con el cambio fallido; y el worker que falla también (§3.1).
...
flowchart LR
  ...
  J --> J4["Contenido · criterio del juez de capítulo"]
...
| Continuidad dura | Antes de escribir cada capítulo: al registrar la escaleta y al ensamblar | Determinista contra ficha y biblia a fecha N−1 (R-2) | Alta | La orden es `error` con causa `inconsistencia`; no gasta intento |
...
Los hallazgos medios y bajos solo se registran y viajan al intento siguiente en el bloque «Informe del intento anterior» (§6.3): no hay regeneración de pasaje ni una pasada extra del Editor. La continuidad del texto, tras R-2, la miran el juez de capítulo y Lean, no una heurística sobre la prosa. Verificación de acto y Originalidad quedan fuera de la v1.
...
3. Nadie aparece en un evento fechado antes de nacer (TC-2): la v1 no compara edades explícitas.

Los momentos van en dos escalas (B-6): los recuerdos del comprador como intervalo de días de calendario, y la historia como `dia-N` más una franja opcional; toda la historia es posterior a todos los recuerdos. Mientras el proyecto Lake no acepte la franja opcional, un evento sin franja se emite como `manana` (M-14).

Las invariantes viven en un proyecto Lake del repositorio (`formal/lean`); el fichero de cada pasada se escribe en `verificacion/pasada-k/cronologia.lean` y se copia a `export/vN/` al publicar (§6). Se ejecuta como gate y no por capítulo. Tras R-2, ningún determinista mira la presencia en el texto: la 1 y la 2 sobre la prosa las ve solo Lean, a partir de los eventos que registra el Bibliotecario. Si Lean no está instalado, o su salida no se puede leer, la orden es `error` con causa `lean_no_disponible`, no un gate rojo.

### 4.3 Guardarraíl de palabras prohibidas

Tres listas: **global** (insultos y términos ofensivos), **por público** (lista adicional para infantil; la de juvenil se usa también en `crossover`) y **por novela** (los vetos del comprador, `definitions.md` §9). La global y las de público son ficheros versionados en `backend/capitulo/listas/`, copiados a `palabra_prohibida` con su hash en `lista_guardarrail`; los vetos se materializan al salir de `contexto`. Antes de comparar se normaliza el texto: minúsculas, sin tildes ni diéresis conservando la ñ, variantes de género y número en palabras de 4 letras o más, y comparación por palabras enteras (B-11), con código propio y sin dependencias. `lenguaje.prohibidas` del contexto va a la lista negra de estilo, no aquí. Cada coincidencia queda en el audit log con el id de la palabra y su posición, **sin el término literal** (B-12). Las muletillas de estilo de la guía de estilo son otra cosa y siguen en Métricas de estilo, con severidad baja.
...
sequenceDiagram
  ...
  O->>W: prompt
  W-->>K: borrador · lo registra el hook SubagentStop
  O->>K: siguiente orden
  ...
  alt fallo bloqueante o alto
    ...
    alt fallo
      ...
      O->>L: capítulo
      L->>K: hechos, uso de hechos y resumen · por /mcp/escritura con el sello
...
flowchart LR
  ...
  BIB --> B3["Cronología · eventos con recuerdo o día de la historia, lugar y presentes"]
  ...
  PRJ --> GT["Gates · resultado por pasada"]
```
...
**Versión de capítulo y versión de novela no son lo mismo.** Un capítulo acumula versiones por intento y por regeneración; una **versión de novela** es una selección publicada: apunta a una versión concreta de cada uno de los 10 capítulos y guarda qué cambio del lector la originó. Publicar la N+1 copia los punteros de los capítulos que no cambiaron, y **la N no se modifica nunca**. Qué capítulos cambiaron es la diferencia entre punteros. Cada versión publicada deja en `export/vN/` seis ficheros —`manuscrito.md`, `metadatos.json`, `lectura.html` (autocontenida, con anclas `#cap-NN`; es la fuente del PDF), `lectura.json` (la que sirve la API a React, con la ficha de personajes y lugares), `novela.pdf` y `cronologia.lean`—, así que se puede leer entera sin reconstruir la biblia. La publicación es atómica: se escribe en un directorio temporal, se renombra a `export/vN` y después se escriben las filas; un `export/vN` sin fila no cuenta como versión.
...
- Al **cerrar un acto**, el Bibliotecario de su último capítulo destila los resúmenes de sus capítulos en un único resumen de acto (B-17, tope de 360 palabras), que dejan de enviarse individualmente. Regenerar un capítulo de un acto cerrado reescribe también el resumen del acto.
...
|---|---|---|
| Ficha del capítulo, con vetos, frases literales y lista negra | ~2k | 1 — nunca se recorta |
| Informe del intento anterior | ~1k | 2 — se recorta entero, antes que la guía |
| Guía de estilo | ~5k | 2 |
| Presagios pendientes, inventario vivo y estado de los presentes | ~2k | 3 |
...
Un bloque obligatorio vacío (ficha, guía, fichas de presentes, localización) es un error; uno opcional vacío (presagios e inventario, resumen, capítulo anterior, reglas) se emite con su encabezado y la línea fija «Ninguno.», que no cuenta como recorte (B-8). Si el capítulo N−1 no está aprobado, se usa el último aprobado y una frase fija lo avisa. Si hubo recortes, el prompt termina con «## Omitido por espacio».
...
De ahí que el recorte sea **discreto y finito**: se retiran unidades en orden inverso hasta que el ensamblado cabe, y el caso límite es quedarse solo con la ficha del capítulo, que nunca se recorta. Qué hacer si ni siquiera eso cabe **está sin decidir** (D-9), y el código no lo puede inventar por su cuenta: mientras tanto, la orden es `error` con causa `no_cabe` y no se envía nada.
...
El prompt se guarda entero para auditoría y, además, **en partes de 20.000 tokens estimados como mucho** (`…prompt.parte-N.md`), cortadas por párrafo y después por línea, cuya concatenación es el fichero entero: `Read` corta hacia los 25.000 tokens, así que la orden del Escritor lleva en `rutas_prompt` la lista de rutas absolutas de las partes, en orden.

Además del prompt, el Recuperador devuelve un **desglose por bloque**: identificador, tokens finales, tamaño esperado y, si hubo recorte, cuánto quedó fuera; más el total de entrada, la lista ordenada de recortes aplicados, el SHA-256 del prompt y el motor del estimador que contó. El bloque de similitud declara también de qué fragmentos salió, con capítulo y posición de cada uno. Esa es la única traza que conecta un fallo de continuidad con su causa: sin ella, cuando el capítulo 8 contradice al 3, no hay forma de saber si fue por un pasaje recuperado.
...
- **Respaldo en Python puro** (M-10). Si la extensión nativa de `tiktoken` no carga —en la máquina de desarrollo la bloquea una directiva de control de aplicaciones de Windows—, el estimador usa un BPE propio sobre el **mismo** fichero `o200k_base`, con la pre-tokenización hecha con `re` y el mismo factor de 1,35. Reproduce los ids de o200k; un pre-token de más de 256 bytes cuenta un token por byte, que es una cota superior. La decisión sigue siendo `tiktoken`: el respaldo vive detrás de la misma interfaz, y la prueba de paridad entre los dos caminos corre donde la extensión carga.
...
| Frontend | React con Vite: `react`, `react-dom`, `vite` y `@vitejs/plugin-react`, con npm (F-1) |
...
**Cómo se registra un resultado.** Cada rebanada que valida la salida de un agente expone `MANEJADORES` (agente → manejador) en `<rebanada>/manejadores.py`: `planificacion`, `escaleta`, `capitulo` (Escritor, Editor, juez de capítulo, Bibliotecario y Revisor), `verificacion` (juez de manuscrito), `exportacion` y `cambio`. `proyecto/manejadores.py` los agrega —y guarda los del Agente de Contexto y el Extractor— y `/resultado` despacha por agente; `proyecto/` no conoce los modelos de salida de nadie. `revision/` guarda solo la revisión humana del manuscrito. El worker de `cambio/` arranca en el `lifespan` de la aplicación (`backend/app.py`).
...
.claude/
  ...
  estado/            · estado local, ignorado por git

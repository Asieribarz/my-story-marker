...
La documentación de proceso —[spec-inicial](docs/proceso/spec-inicial.md), [trade-offs](docs/proceso/trade-offs.md), [explainers](docs/proceso/explainers.md), [diagramas](docs/proceso/diagramas.md), [uso de Claude Code](docs/proceso/claude-code.md), [evals](docs/evals.md), [iteraciones](docs/iteraciones.md) y [red-team](docs/red-team.md)— sigue a estos cuatro: si se contradicen, mandan ellos.
...
Cada brief de evaluación tiene su carpeta, `evals/eN-<nombre>/`, con nombres en inglés y siempre los mismos: `input/` guarda `brief.json`, `reference-context.json` y, si lo hay, el texto libre (que conserva el prefijo `texto_libre` para que la policy lo siga bloqueando); `results/` guarda un informe por ejecución, `<AAAA-MM-DD>.md`, con la entrada, qué capa detectó el error y con qué evidencia, y el resultado frente al oráculo del brief. Los ataques de E2 van además a [docs/red-team.md](docs/red-team.md). El informe conjunto de una ronda de evaluaciones va en `evals/report.md`, y la tabla resumen en [docs/evals.md](docs/evals.md).
...
Todo lo demás (el mecanismo de búsqueda dentro de SQLite, la observabilidad y el despliegue) está sin decidir — ver [docs/architecture.md](docs/architecture.md) §7. No introduzcas ninguna de esas dependencias por iniciativa propia: propón la decisión, y si se acepta, añádela a esa tabla en el mismo cambio.
